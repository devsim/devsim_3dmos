...
def CreateElementElectronContinuityEquation(device, region, current_model):
    ...
    '''
    ds.equation(device=device, region=region, name="ElectronContinuityEquation", variable_name="Electrons",
             time_node_model="NCharge", node_model="ElectronGeneration",
             element_model=current_model, variable_update="positive")

def CreateElementHoleContinuityEquation(device, region, current_model):
    '''
      Uses element current model for equation
    '''
    ds.equation(device=device, region=region, name="HoleContinuityEquation", variable_name="Holes",
...
#TODO: expand for circuit
def CreateElementContactHoleContinuityEquation(device, contact, current_model):
    '''
      Uses element current model for equation
    '''
...
def CreateNormalElectricFieldFromInterfaceNormal(device, region, interface):
    ...
    if dimension == 3:
        Enormal="{0}_normal_x * ElectricField_x + {0}_normal_y * ElectricField_y + {0}_normal_z * ElectricField_z".format(interface)
    elif dimension == 2:
...
def CreateNormalElectricFieldFromCurrentFlow(device, region, low_curr):
    do_j_derivatives = False
    dimension=ds.get_dimension(device=device)

    ds.element_from_edge_model(edge_model=low_curr, device=device, region=region)
    if do_j_derivatives:
        ds.element_from_edge_model(edge_model=low_curr, device=device, region=region, derivative="Potential")
        ds.element_from_edge_model(edge_model=low_curr, device=device, region=region, derivative="Electrons")
        ds.element_from_edge_model(edge_model=low_curr, device=device, region=region, derivative="Holes")
    ...
    if dimension == 2:
        J_lf_mag="pow({0}_x^2 + {0}_y^2, 0.5)".format(low_curr)
        #J_lf_mag="pow({0}_x^2 + {0}_y^2 + 1e-300, 0.5)".format(low_curr)
        ens = ("@en0", "@en1", "@en2")
        exs = "-({0}_x * {0}_x:{1}{2} + {0}_y * {0}_y:{1}{2})/{0}_mag"
        dirs = ("x", "y")
        ...
        exef = "-(ElectricField_x * ElectricField_x:{0}{1} + ElectricField_y * ElectricField_y:{0}{1})/ElectricField_mag"
    elif dimension == 3:
        J_lf_mag="pow({0}_x^2 + {0}_y^2 + {0}_z^2, 0.5)".format(low_curr)
        #J_lf_mag="pow({0}_x^2 + {0}_y^2 + {0}_z^2 + 1e-300, 0.5)".format(low_curr)
        ens = ("@en0", "@en1", "@en2", "@en3")
        exs = "-({0}_x * {0}_x:{1}{2} + {0}_y * {0}_y:{1}{2} + {0}_z * {0}_z:{1}{2})/{0}_mag"
        dirs = ("x", "y", "z")
        # this is to test the EField derivatives by themselves
        #Eparallel_J="unitx * ElectricField_x + unity * ElectricField_y + unitz * ElectricField_z"
        Eparallel_J="{0}_norm_x * ElectricField_x + {0}_norm_y * ElectricField_y + {0}_norm_z * ElectricField_z".format(low_curr)
        ElectricField_mag="pow(ElectricField_x^2 + ElectricField_y^2 + ElectricField_z^2 + 1e-300, 0.5)"
        exef = "-(ElectricField_x * ElectricField_x:{0}{1} + ElectricField_y * ElectricField_y:{0}{1} + ElectricField_z * ElectricField_z:{0}{1})/ElectricField_mag"
    else:
        raise RuntimeError("dimension %d not supported" % dimension)
