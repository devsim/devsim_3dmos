...
def CreateOxideContact(device, region, contact, vshift=""):
    conteq="Permittivity*ElectricField"
    contact_bias_name  = GetContactBiasName(contact)
    contact_model_name = GetContactNodeModelName(contact)
    if vshift:
        eq = "Potential - {0} - {1}".format(GetContactBiasName(contact), vshift)
    else:
...
def SetOxideParameters(device, region, T):
    ...
    '''
    ds.set_parameter(device=device, region=region, name="Permittivity",   value=eps_ox * eps_0)
...
def SetSiliconParameters(device, region, T):
    ...
    ds.set_parameter(device=device, region=region, name="Permittivity",   value=eps_si * eps_0)
    ds.set_parameter(device=device, region=region, name="ElectronCharge", value=q)
    ...
    ds.set_parameter(device=device, region=region, name="taun", value=1e-5)
    ds.set_parameter(device=device, region=region, name="taup", value=1e-5)
...
def CreateSiliconSiliconInterface(device, interface):
    ...
    ds.interface_equation(device=device, interface=interface, name="ElectronContinuityEquation", variable_name="Electrons", interface_model=ename, type="continuous")
    hname = CreateContinuousInterfaceModel(device, interface, "Holes")
    ds.interface_equation(device=device, interface=interface, name="HoleContinuityEquation", variable_name="Holes", interface_model=hname, type="continuous")
